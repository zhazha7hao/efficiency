...
def main():
    # 生成纬度任务列表 (-90到90度共181个纬度)
    all_latitudes = [(str(lat), lat) for lat in range(-90, 91)]
    
    # 将任务分成4个部分
    num_parts = 4
    part_size = len(all_latitudes) // num_parts
    parts = []
    for i in range(num_parts):
        start = i * part_size
        end = (i + 1) * part_size if i < num_parts - 1 else len(all_latitudes)
        parts.append(all_latitudes[start:end])
    
    # 让用户选择要运行的部分
    print("请选择要运行的任务部分:")
    for i in range(num_parts):
        start_lat = parts[i][0][1]
        end_lat = parts[i][-1][1]
        print(f"{i+1}. 纬度范围: {start_lat}° 到 {end_lat}°")
    
    while True:
        try:
            choice = int(input("请输入要运行的部分(1-4): "))
            if 1 <= choice <= 4:
                break
            print("请输入1到4之间的数字!")
        except ValueError:
    ...
    selected_part = parts[choice-1]
    
    # 顺序处理选定部分中的每个纬度，每个纬度内部并行
    results = []
    for lat_info in selected_part:
    ...
    # 保存结果
    output_file = f'单纬度并行优化结果_部分{choice}.xlsx'
